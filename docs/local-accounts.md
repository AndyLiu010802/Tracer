...
- 动效第四版移除了照片条带的往返平移：水面使用二维行进波法线计算局部折射，云雾使用单向流动与双阶段渐变衔接，火焰提取暖色发光区域后向上输运，极光只改变光帘亮度，保留固定景物。雨滴加速下滑，雨雪与花瓣持续下落，萤火采用不规则路径。`wallpaper-flow.js` 复用一个 WebGL 画布与最多四张纹理；不可用时回退到静止底景上的波纹和粒子。新增固定景物像素及流动循环连续性检查，对比视频为 `design/dynamic-wallpapers-v2/natural-motion-comparison.mp4`。
