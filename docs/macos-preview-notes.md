...
**Desktop release** 工作流会原生构建并检查 Windows、Mac arm64 和 Mac x64，三个平台全部成功后才发布统一附件。下载附件尚未出现时，表示构建或发布未完成；本次云端检查结果以工作流记录为准，本说明不代表 Mac 检查已经运行通过。本地 Windows 0.3.6 已通过真实打包应用在全新用户目录中的启动与交互检查。Mac 界面实机操作、真实账号 AI 登录及后台输入权限仍待验收。
