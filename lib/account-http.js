  ...
  return async function handle(req, res, pathname) {
    ...
    try {
      ...
      if (!['session','capabilities'].includes(action)) {
        const context = await provider.context(token);
        if (req.headers['x-tracer-scope'] !== context.scope || String(req.headers['x-tracer-generation'] || 0) !== String(context.generation || 0)) { req.resume?.(); send(res,409,{error:'account-changed'}); return true; }
      }
      if (req.method === 'GET') {
      ...
      operations['clear-data'] = () => provider.clearData(token,input,req.headers['user-agent']);
