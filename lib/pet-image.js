...
function sameGeneratedPath(supplied, actual) {
...
async function generatedFile(room, file, { createdAfter = 0 } = {}) {
  ...
  try {
    ...
    // Reject redirected roots and descendants, except the exact macOS system aliases.
    if (!sameGeneratedPath(room, root) || !sameGeneratedPath(file, target)) throw new Error('invalid-image-response');
    let cursor = root;
