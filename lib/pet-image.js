...
async function generatedFile(room, file, { createdAfter = 0 } = {}) {
  ...
  try {
    ...
    // Reject redirected roots as well as links inside the native output directory.
    if (path.relative(path.resolve(room), root) || path.relative(path.resolve(file), target)) throw new Error('invalid-image-response');
    let cursor = root;
