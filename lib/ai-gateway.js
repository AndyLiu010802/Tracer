...
function createGateway(dir,{request=fetch}={}){
 ...
 return {async handle(action,data){if(action.startsWith('personal-'))return personal.handle(action,data);read();if(action==='status'){let user=null,error=null;if(token)try{user=(await remote('me')).user;}catch(e){error=e.message;}return {url:connection?connection.url:'',user,error,persistentLogin:!!(secure&&secure.isEncryptionAvailable())};}
...
module.exports={createGateway,setSecureStorage,endpoint};
