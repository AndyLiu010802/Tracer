...
function createGateway(dir,{request=fetch}={}){
 ...
 return {async handle(action,data){read();if(action==='status'){let user=null,error=null;if(token)try{user=(await remote('me')).user;}catch(e){error=e.message;}return {url:connection?connection.url:'',user,error,persistentLogin:!!(secure&&secure.isEncryptionAvailable())};}
