...
function inspect(raw) {
  ...
  } else if (!keys(art, ['layout', 'animationVersion', 'images']) || art.layout !== 'atlas-4x4' || art.animationVersion !== 2 ||
  ...
  const id = 'custom_' + hash(JSON.stringify({ profile, layout: art.layout,
    ...(animationVersion === 2 ? { animationVersion } : {}), images: images.map(hash) })).slice(0, 32);
  return { profile: { id, ...profile }, animated, animationVersion,
...
async function exportPackage(dir, raw) {
  ...
  return { format: 'tracer-companion', version: animationVersion === 2 ? 2 : 1, companion: metadata,
    artwork: { layout: animated ? 'atlas-4x4' : 'portrait', ...(animationVersion === 2 ? { animationVersion } : {}), images } };
}
