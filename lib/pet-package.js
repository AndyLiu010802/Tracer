...
function retained(raw) {
  if (!Array.isArray(raw) || raw.length !== 16 || !Array.from({ length: 16 }, (_, index) => index).every(index => [1, 4, 16].includes(raw[index]))) throw invalid();
  return raw.some(count => count !== 16) ? raw.slice() : undefined;
}
// Keep rendering chunks only: source paths, prompts and other PNG text metadata are not shared.
function artwork(bytes, animated) {
  Images.generatedBytes(bytes, { animation: animated });
...
function inspect(raw) {
  ...
  if (![1, 2].includes(raw.version)) throw new Error('unsupported-pet-package');
  ...
  } else if (!keys(art, ['layout', 'animationVersion', 'images', ...(art && Object.hasOwn(art, 'retainedFrames') ? ['retainedFrames'] : [])]) || art.layout !== 'atlas-4x4' || art.animationVersion !== 2 ||
      !Array.isArray(art.images) || art.images.length !== 16) throw invalid();
  const animated = art.layout === 'atlas-4x4', images = [];
  const animationVersion = animated ? raw.version : 0;
  const retainedFrames = raw.version === 2 && Object.hasOwn(art, 'retainedFrames') ? retained(art.retainedFrames) : undefined;
  for (const item of art.images) {
    ...
    try { images.push(artwork(bytes, animated)); } catch { throw invalid(); }
  }
...
async function exportPackage(dir, raw) {
  ...
  for (const url of animated ? profile.animation.pages : [profile.image]) {
    ...
    const clean = artwork(bytes, animated);
    images.push({ sha256: hash(clean), data: clean.toString('base64') });
  }
  const animationVersion = profile.animation?.version;
  return { format: 'tracer-companion', version: animationVersion === 2 ? 2 : 1, companion: metadata,
    artwork: { layout: animated ? 'atlas-4x4' : 'portrait', ...(animationVersion === 2 ? { animationVersion } : {}),
