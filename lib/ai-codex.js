...
function runtimePath() {
  const bundled = process.resourcesPath && path.join(process.resourcesPath, 'codex', 'bin', 'codex.exe');
  if (bundled && fs.existsSync(bundled)) return bundled;
  return path.join(__dirname, '../.cache/codex-runtime/package/vendor/x86_64-pc-windows-msvc/bin/codex.exe');
}
...
function childEnvironment(home) {
  const env = {};
  for (const name of ['SYSTEMROOT','WINDIR','PATH','TEMP','TMP','USERPROFILE','APPDATA','LOCALAPPDATA','PROGRAMDATA','ALLUSERSPROFILE','HOMEDRIVE','HOMEPATH','HTTP_PROXY','HTTPS_PROXY','NO_PROXY']) {
