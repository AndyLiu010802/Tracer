...
function createCodex(dir, { executable = runtimePath(), spawn = cp.spawn, timeout = 30000, turnTimeout = 180000, imageTimeout = 300000,
  ...
  async function generate(connection, raw, isTest = false, isChat = false) {
    ...
    const instructions = isChat ? Companion.instructions(data) + ' Return one JSON object with a reply string.' : 'You are a concise task planning assistant in Tracer. Use only the supplied text. Never use tools, browse, inspect files, run commands, or delegate. ' +
    ...
    try {
      ...
      if (isChat) return Companion.reply(parsed.reply);
    ...
    try {
      ...
      const thread = await connection.request('thread/start', {
        ...
        baseInstructions: (profile.animationPage === undefined ? 'Create exactly one image' : profile.animationVersion === 2 ? 'Create exactly one animation sprite sheet with four rows and four columns containing sixteen consecutive distinct poses of ONE action, in row-major reading order' : 'Create exactly one animation sprite sheet with four rows and four columns, following the exact action and frame layout') +
          ...
          'Request high quality, a transparent PNG background and square 1024x1024 output. Do not simulate the result with SVG, code, a text description or an existing image. ' +
      ...
      bytes = Images.generatedBytes(bytes, { animation: profile.animationPage !== undefined });
      return { image: await Images.storeGenerated(dir, bytes), model: 'codex-image-generation', ...(profile.animationPage !== undefined ? { animationPage: profile.animationPage, ...(profile.animationVersion === 2 ? { animationVersion: 2 } : {}) } : {}) };
    } finally {
  ...
  return {
    close() { rpc?.close(); imageRpc?.close(); },
