...
function createLocalAccounts(dataDir, { now = Date.now, sessionLifetime = 30 * DAY, beforeClear = async () => {} } = {}) {
  ...
  return {
    ...
    async context(token) {
      ...
      const account = value.accounts.find(a => a.id === current.accountId);
      return { scope: current.accountId, generation: account.generation || 0, user: publicUser(account), capabilities };
    },
    async beginDataRequest(token, expectedScope, generation, allowUnscoped = false) {
      return serial(async () => {
        const value = await read(), current = session(value, token);
    ...
    async clearData(token, input, agent) {
