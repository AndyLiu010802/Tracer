  ...
  return { async handle(action, data = {}) {
    ...
    try {
      ...
      if (action === 'personal-chat') {
        ...
        const body = config.protocol === 'responses' ? { model: config.model, store: false, instructions, input: input.messages, max_output_tokens: 800 } :
          { model: config.model, messages: [{ role: 'system', content: instructions }, ...input.messages] };
