...
async function main(){
  ...
  try{
    ...
    for(const check of checks)assert.equal(check.unique,16,check.id+':'+check.action+' must have sixteen different rendered images');
    for(const id of ['sprout','miso','brook','ember','luna','nova']){
      await page.evaluate(id=>{
        ...
        document.body.innerHTML=TracerPetBuiltinAnimation.actions.map(action=>[0,4,8,12].map(frame=>'<section style="background:#17201b;text-align:center"><div>'+id+' · '+action+' · '+frame+'</div>'+TracerPetBuiltinAnimation.snapshot(id,action,frame).replace('<svg ','<svg width="136" height="136" ')+'</section>').join('')).join('');
      },id);
    ...
    for(const name of ['pet.css','pet-interactions.css','pet-idle.css','pet-desktop.css','pet-animation.css','pet-character-motion.css'])await page.addStyleTag({path:path.join(root,'skins/tracer',name)});
    ...
    const start=await page.locator('.pet-builtin-sprite').getAttribute('data-frame');await page.waitForTimeout(480);assert.notEqual(await page.locator('.pet-builtin-sprite').getAttribute('data-frame'),start);
    ...
    console.log('PASS 1,536 raster-distinct poses, all 96 loops, frame bounds, real CSS playback and reduced motion. Artifacts: '+folder);
