(function(root,factory){
  ...
  function create(options={}){
    ...
    function refresh(){if(dead)return;if((doc.hidden || doc.tracerHidden)||media.matches||!visible)cancelPreview();draw();schedule();}
    ...
    if(options.animated!==false){['visibilitychange','tracer-visibilitychange'].forEach(event=>doc.addEventListener(event,refresh));media.addEventListener('change',refresh);observer?.observe(element);}
    draw();schedule();
    return{element,availableActions:()=>actions.slice(),
      ...
      preview(value,callback){if(dead||!actions.includes(value)||['sleep','focus','drag'].includes(base)||(doc.hidden || doc.tracerHidden)||media.matches||!visible||options.animated===false)return false;
      ...
      destroy(){if(dead)return;dead=true;stop();finish=null;observer?.disconnect();layer.destroy();['visibilitychange','tracer-visibilitychange'].forEach(event=>doc.removeEventListener(event,refresh));media.removeEventListener('change',refresh);element.dataset.playback='destroyed';}
    };
  }
  return{create};
});
