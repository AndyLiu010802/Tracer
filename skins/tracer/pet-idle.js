(function (root, factory) {
  ...
  function create() {
    ...
    function update(input, now = Date.now()) {
      ...
      if (anchor === null || changed || wasBlocked || interrupted || elapsed < 0 || !Number.isFinite(elapsed)) anchor = now;
      ...
      return order[((preferred?0:offset(petId)) + Math.floor(age / cycleTime)) % order.length];
    }
    return { update, reset };
  }
