(function(root,factory){
  ...
  const catalog=[
    ...
    {id:'neon_orchid',name:['霓虹蝶兰','Holographic orchid'],description:['品红蝶兰与青蓝棱光交织，留下双层全息光轨。','Magenta orchids and cyan facets weave a double holographic light trace.'],colors:['#f5a6ff','#5be6f3','#c6b4ff'],life:900,spacing:18,fall:-9,sway:9,ribbon:'neon'},
  ...
  function glyph(doc,p,type){
    ...
    if(type===3){star(c,4.5);return canvas;}
    const r=12;
    switch(p.id){
      case 'wildflower':
      ...
      case 'neon_orchid':
        if(type===0){for(let i=0;i<5;i++){c.save();c.rotate(i*TAU/5);c.fillStyle=i%2?rgba(colors[1],.6):rgba(colors[0],.85);c.strokeStyle=i%2?colors[1]:colors[0];c.beginPath();c.moveTo(0,2);c.lineTo(-7,-8);c.lineTo(-2,-15);c.lineTo(4,-8);c.closePath();c.fill();c.stroke();c.restore();}c.fillStyle='#fff7ff';star(c,5);}
        else if(type===1){c.strokeStyle=colors[1];c.lineWidth=1.4;c.beginPath();c.moveTo(0,-9);c.lineTo(6,0);c.lineTo(0,9);c.lineTo(-6,0);c.closePath();c.stroke();c.fillStyle='#ffe2ff';star(c,3);}
        else{c.strokeStyle=colors[0];c.beginPath();c.ellipse(0,0,11,4,-.5,0,TAU);c.stroke();}break;
      case 'volt_berry':
  ...
  function create(canvas,options={}){
    ...
    let theme=profile(options.kind)||catalog[0],sprites=[],particles=[],knots=[],last=null,carry=0,serial=0,raf=null,previous=null,lastPaint=-Infinity,destroyed=false,width=1,height=1;
    const animate=options.animate!==false;
    const now=()=>win.performance.now();
    function reset(){if(raf!==null)win.cancelAnimationFrame(raf);raf=null;c.clearRect(0,0,width,height);particles=[];knots=[];last=null;carry=0;previous=null;lastPaint=-Infinity;}
    ...
    function resize(w,h,dpr=1){reset();width=Math.max(1,w);height=Math.max(1,h);const ratio=Math.min(2,Math.max(1,dpr));canvas.width=Math.round(width*ratio);canvas.height=Math.round(height*ratio);c.setTransform(ratio,0,0,ratio,0,0);}
    ...
    function point(value,at=now()){
      ...
      if(last){const dx=x-last.x,dy=y-last.y,distance=Math.hypot(dx,dy);
        if(distance>700){reset();}else if(distance>0){
          ...
          while(step<=distance&&emitted<28){const t=step/distance,n=++serial,r=random(n),side=random(n+190)-.5;
            ...
            particles.push({x:last.x+dx*t+nx*side*9,y:last.y+dy*t+ny*side*9,at,life:theme.life*(.8+random(n+71)*.35),type,
              size:type===0?25+r*8:type===3?8+r*5:14+r*9,angle:Math.atan2(dy,dx)+r*TAU,spin:(random(n+83)-.5)*3.2,phase:r*TAU,nx,ny,drift:side*30});
            step+=spacing;emitted++;
          }
          carry=(carry+distance)%spacing;
        }
      }
      if(!last||x!==last.x||y!==last.y)knots.push({x,y,at});
      last={x,y};if(particles.length>120)particles.splice(0,particles.length-120);if(knots.length>32)knots.splice(0,knots.length-32);schedule();
    }
    function ribbon(at){
      if(!theme.ribbon||knots.length<2)return;
      const rows=knots.filter(p=>at-p.at<520);if(rows.length<2)return;
      for(let layer=0;layer<2;layer++){
        c.beginPath();rows.forEach((p,i)=>{const shift=theme.ribbon==='electric'?(i%2?1:-1)*4:Math.sin(i*.68+layer*2.5)*3;
          const x=p.x,y=p.y+shift+(layer?2:-2);i?c.lineTo(x,y):c.moveTo(x,y);});
        ...
        c.globalAlpha=Math.max(0,1-(at-b.at)/520);c.strokeStyle=g;c.lineCap='round';c.lineJoin='round';c.lineWidth=theme.ribbon==='silk'?(layer?3:7):theme.ribbon==='aurora'?(layer?1:4):theme.ribbon==='electric'?1.2:1.4;c.stroke();
      }
      c.globalAlpha=1;
    }
    function draw(at=now()){
      ...
      particles=particles.filter(p=>at-p.at<p.life);knots=knots.filter(p=>at-p.at<520);
      let minX=Infinity,minY=Infinity,maxX=-Infinity,maxY=-Infinity;
      function bounds(x,y,r){minX=Math.min(minX,x-r);minY=Math.min(minY,y-r);maxX=Math.max(maxX,x+r);maxY=Math.max(maxY,y+r);}
      knots.forEach(p=>bounds(p.x,p.y,12));ribbon(at);
      for(const p of particles){const age=Math.max(0,(at-p.at)/p.life),ease=1-Math.pow(1-age,2);
        ...
        const alpha=Math.min(1,(1-age)*2.1),size=p.size*(1-age*.24);
        c.save();c.translate(x,y);c.rotate(p.angle+p.spin*age);c.globalAlpha=alpha*.92;
        if(['cherry','peach','wildflower'].includes(theme.id)&&p.type!==3)c.scale(.65+.35*Math.abs(Math.cos(age*4+p.phase)),1);
        if(theme.id==='volt_berry'&&p.type===2)c.scale(1+age*.7,1+age*.7);
        c.drawImage(sprites[p.type],-size,-size,size*2,size*2);c.restore();bounds(x,y,size*2+3);
      }
      previous=minX===Infinity?null:{x:Math.floor(minX)-2,y:Math.floor(minY)-2,width:Math.ceil(maxX-minX)+4,height:Math.ceil(maxY-minY)+4};
      c.globalAlpha=1;
    }
    function tick(at){raf=null;if(at-lastPaint>=32){draw(at);lastPaint=at;}if(particles.length||knots.length)schedule();}
    setKind(theme.id);
